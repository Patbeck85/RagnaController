...
namespace RagnaController
{
    public partial class MacroRecorderWindow : Window
    {
        ...
        public MacroRecorderWindow()
        {
            ...
            // Always clean up on close – handles both the X button and BtnClose
        ...
        {
            // Stop recording if still active (user hit X without confirming)
            if (_recorder.IsRecording)
                _recorder.CancelRecording();

            // Unsubscribe so the recorder can't fire events on this dead window
            _recorder.RecordingStarted -= OnRecordingStarted;
            _recorder.RecordingStopped -= OnRecordingStopped;
            _recorder.StepRecorded     -= OnStepRecorded;
        }
        ...
        private void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                // Save to AppData via MacroRecorder (consistent storage location)
                string filePath = MacroRecorder.SaveMacro(_recordedMacro);
                ...
                {
                    var editor = new MacroEditorWindow(filePath) { Owner = this };
                    editor.ShowDialog();
                }
        ...
        {
            if (index < 0 || index >= StepsList.Items.Count) return;
            StepsList.Items.RemoveAt(index);
            // Renumber remaining steps
            for (int i = index; i < StepsList.Items.Count; i++)
            {
                if (StepsList.Items[i] is System.Windows.Controls.Grid g &&
                    g.Children.Count > 0 &&
                    g.Children[0] is System.Windows.Controls.TextBlock tb)
                {
                    tb.Text = (i + 1).ToString();
                    // Update delete button tag
                    if (g.Children.Count > 3 &&
                        g.Children[3] is System.Windows.Controls.Button btn)
                        btn.Tag = i;
                }
            }
