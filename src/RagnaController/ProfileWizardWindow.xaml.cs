...
namespace RagnaController
{
    public partial class ProfileWizardWindow : Window
    {
        ...
        private void UpdateStepDisplay()
        {
            ...
            // Reset dots
            var inactiveBrush = (Brush)(FindResource("BG3Brush") ?? Application.Current.Resources["BG3Brush"]);
            var activeBrush   = (Brush)(FindResource("NeonBrush") ?? Application.Current.Resources["NeonBrush"]);
        ...
        {
            // Guard: event can fire during InitializeComponent() before CheckBoxes exist
        ...
        private void CreateProfile()
        {
            try
            {
                ...
                // Pre-fill skill recommendations based on selected class
        ...
        private static void ApplyClassDefaults(Profile p)
        {
            (p.SkillRecommendations, p.ClassTips) = p.Class switch
            {
                "Melee" => (new System.Collections.Generic.List<string>
                {
                    "F1  → Main attack / Bash",
                    "F2  → Defensive skill",
                    "F3  → Buff (Berserk / Provoke)",
                    "F4  → Potion / Heal item",
                    "F5  → Secondary attack",
                    "Ctrl+F1 → Ultimate skill"
                },
                "Hold L2 for modifier layer.\nR2 activates Ctrl+F skills for the second skill row."),

                "Ranged" => (new System.Collections.Generic.List<string>
                {
                    "F1  → Main skill (arrow / shot)",
                    "F2  → AoE skill",
                    "F3  → Set trap",
                    "F4  → Buff (Sharp Shooting etc.)",
                    "F5  → Evasion skill",
                    "Ctrl+F1 → Ultimate / cooldown skill"
                },
                "Kite engine active: controller automatically maintains distance.\nToggle kite mode with L3."),

                "Mage" => (new System.Collections.Generic.List<string>
                {
                    "F1  → Main spell (Bolt / Storm)",
                    "F2  → AoE spell",
                    "F3  → Defensive / Safety Wall",
                    "F4  → Buff (Mystical Amplification)",
                    "F5  → SP recovery item",
                    "Ctrl+F1 → Ultimate spell"
                },
                "Mage system active: right stick aims ground spells.\nCursor speed of 800-1000 recommended for precise targeting."),

                "Support" => (new System.Collections.Generic.List<string>
                {
                    "F1  → Heal",
                    "F2  → Buff (Blessing / AGI Up)",
                    "F3  → Resurrection",
                    "F4  → Sanctuary / AoE Heal",
                    "F5  → Self-heal",
                    "Ctrl+F1 → Emergency skill"
                },
                "Support mode: L3 for party targeting.\nPoint cursor at ally -> F1 for direct heal."),

                _ => (new System.Collections.Generic.List<string>
                {
                    "Assign F-keys freely in-game.",
                    "Use L1/R1/L2/R2 layers for more buttons.",
                    "Open the remap window to customize."
                },
                "Custom profile - assign F-keys to match your class.\nUse the remap window to customize all buttons.")
            };
        }
